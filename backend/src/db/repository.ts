...
export class Repository {
  ...
  async createUser(email: string, passwordHash: string): Promise<User> {
    ...
    if (pool) {
      const res = await pool.query(
        `INSERT INTO users (id, email, password_hash, subscription_status, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
        [id, email, passwordHash, 'INACTIVE', now, now]
      );
      return res.rows[0];
    }
  ...
  async updateUserSubscription(
    ...
    if (pool) {
      await pool.query(
        `UPDATE users SET subscription_status = $1, subscription_id = COALESCE($2, subscription_id), razorpay_customer_id = COALESCE($3, razorpay_customer_id), updated_at = $4 WHERE id = $5`,
        [status, subscriptionId || null, customerId || null, now, id]
      );
      return;
    }
    ...
    if (user) {
      ...
      if (subscriptionId) user.subscription_id = subscriptionId;
